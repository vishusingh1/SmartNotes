...
var require_url = __commonJS({
  "node_modules/.pnpm/hono@3.8.1/node_modules/hono/dist/cjs/utils/url.js"(exports, module2) {
    ...
    var __copyProps2 = (to, from, except, desc4) => {
      if (from && typeof from === "object" || typeof from === "function") {
        for (let key of __getOwnPropNames2(from))
          if (!__hasOwnProp2.call(to, key) && key !== except)
            __defProp2(to, key, { get: () => from[key], enumerable: !(desc4 = __getOwnPropDesc2(from, key)) || desc4.enumerable });
      }
      return to;
    };
...
var require_dist = __commonJS({
  "node_modules/.pnpm/valibot@0.19.0/node_modules/valibot/dist/index.cjs"(exports, module2) {
    ...
    function parse2(schema2, input, info) {
      const result = schema2._parse(input, info);
      if (result.issues) {
...
var sqliteQueryEngine = async (setup) => {
  ...
  const tablesFn = async (schema2, options) => {
    const tablesConfig = setup.schema[schema2];
    const perTablePromises = Object.values(tablesConfig).filter((it) => {
      ...
      if (!options?.withoutCount) {
        const table = schemaToTables[schema2][tableName];
  ...
  return {
    query: (schema2) => queryFn(db, schemaToTables[schema2]),
    insert: (schema2) => insertFn(db, schemaToTables[schema2], schmaToTablesToColumnNames[schema2]),
    update: (schema2) => updateFn(db, schemaToTables[schema2], schmaToTablesToColumnNames[schema2]),
    delete: (schema2) => deleteFn(db, schemaToTables[schema2]),
    dropTable: (schema2) => dropTableFn(db, schemaToTables[schema2]),
    truncate: (_) => () => {
...
var queryFn2 = (db, schema2) => (params) => {
  const tablesMap = Object.values(schema2).reduce((acc, it) => {
...
var pgQueryEngine = async (setup) => {
  ...
  const tables = async (schema2, options) => {
    const tablesConfig = setup.schema[schema2];
    const perTablePromises = Object.values(tablesConfig).filter((it) => {
  ...
  return {
    query: (schema2) => queryFn2(db, schemaToTables[schema2]),
    delete: (schema2) => deleteFn2(db, schemaToTables[schema2]),
    insert: (schema2) => insertFn2(db, schemaToTables[schema2], schmaToTablesToColumnNames[schema2]),
    update: (schema2) => updateFn2(db, schemaToTables[schema2], schmaToTablesToColumnNames[schema2]),
    dropTable: (schema2) => dropTableFn2(db, schemaToTables[schema2]),
    truncate: (schema2) => truncateTableFn(db, schemaToTables[schema2]),
    tables,
    schemas: Object.keys(setup.schema),
    relations: extractRelations(relationsConfig)
  };
...
var mysqlQueryEngine = async (setup) => {
  ...
  return {
    query: (schema2) => queryFn3(db, schemaToTables[schema2]),
    delete: (schema2) => deleteFn3(db, schemaToTables[schema2]),
    insert: (schema2) => insertFn3(db, schemaToTables[schema2], schmaToTablesToColumnNames[schema2]),
    update: (schema2) => updateFn3(db, schemaToTables[schema2], schmaToTablesToColumnNames[schema2]),
    dropTable: (schema2) => dropTableFn3(db, schemaToTables[schema2]),
    truncate: (schema2) => truncateFn(db, schemaToTables[schema2]),
    tables,
    schemas: Object.keys(setup.schema),
    relations: extractRelations(relationsConfig)
  };
...
var prepareServer = async (setup, app) => {
  ...
  const {
    ...
    dropTable,
    truncate
  } = await queryEngineForSetup(setup);
  app.post("/", (0, import_valibot_validator.vValidator)("json", schema), async (c) => {
    const body = c.req.valid("json");
    const { type, schema: schema2 } = body;
    switch (type) {
      case "init":
        return c.json({ version: "1", dialect: setup.type, schemas });
      case "sql-schema":
        const count = body.data?.count ?? true;
        const res = await tables(schema2, {
          withoutCount: !count
        });
        return c.json({ tables: res, relations });
      case "sql-insert": {
        const { table, data } = body.data;
        await insert(schema2)({ table, data });
        return c.json({ status: "ok" });
      }
      case "sql-select": {
        const { table, columns, where, orderBy, limit, offset } = body.data;
        try {
          const result = await query(schema2)({
            table,
            columns,
            where,
            orderBy,
      ...
      case "sql-update": {
        ...
        await update(schema2)({ table, where, data });
        return c.json({ status: "ok" });
      }
      case "sql-delete": {
        const { table, where } = body.data;
        await del(schema2)({ table, where });
        return c.json({ status: "ok" });
      }
      case "sql-truncate": {
        const { table, isCascade } = body.data;
        await truncate(schema2)({ table, isCascade });
        return c.json({ status: "ok" });
      }
      case "sql-drop": {
        const { table, isCascade } = body.data;
        await dropTable(schema2)({ table, isCascade });
        return c.json({ status: "ok" });
      }
      default:
